...
(|||) a b =
    P (\i -> let r = parse a i
              in bool r (parse b i) (isErrorResult r))
...
satisfy =
    ...
    -- error "todo"
    -- \p -> c >>= \a -> bool (p) (unexpectedCharParser a) (p a)
    -- \p -> c >>= \b -> bool (unexpectedCharParser b) (pure b) (p b)
...
instance Applicative Parser where
  ...
  (<*>) pab pa =
    ...
    -- bindParser (\f -> bindParser (\g -> valueParser(f g)) pa) pab
